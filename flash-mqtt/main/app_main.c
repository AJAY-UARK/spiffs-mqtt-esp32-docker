...
#ifndef DEVICE_ID
#define DEVICE_ID "esp-device-default"
#endif

#ifndef TOPIC
#define TOPIC "esp-device-default/data"
#endif
...
static void publisher_task(void *arg)
{
    ...
    char line[128], device[32], value[32];

    /* ---------- read & parse the FIRST line ---------- */
    if (fgets(line, sizeof(line), f) == NULL) {
        ESP_LOGE(TAG, "data.txt empty");
        fclose(f);
        vTaskDelete(NULL);
    }
    ...
    if (sscanf(line, "%31[^,],%31s", device, value) != 2) {
        ESP_LOGE(TAG, "Bad CSV: %s", line);
        fclose(f);
        vTaskDelete(NULL);
    }

    // snprintf(topic, sizeof(topic), "%s/data", device);   /* device1/data */
    const char *topic = TOPIC;   // fixed publish topic
    
    ESP_LOGI(TAG, "MQTT Client ID : %s", DEVICE_ID);
    ESP_LOGI(TAG, "MQTT Publish Topic: %s", TOPIC);

    /* ---------- publish first value ---------- */
    esp_mqtt_client_publish(s_client, topic, value, 0, 1, 0);
    ESP_LOGI(TAG, "First publish: %s → %s", topic, value);
    vTaskDelay(pdMS_TO_TICKS(200));   /* feed WDT */

    /* ---------- publish remaining lines ---------- */
    while (fgets(line, sizeof(line), f) != NULL) {
        len = strcspn(line, "\r\n");  line[len] = '\0';

        char *comma = strchr(line, ',');        /* skip “device,” part */
        const char *payload = comma ? comma + 1 : line;

        int msg_id = esp_mqtt_client_publish(s_client, topic,
                                             payload, 0, 1, 0);
        ESP_LOGI(TAG, "Published: %s → %s (msg_id=%d)",
                 topic, payload, msg_id);

        vTaskDelay(pdMS_TO_TICKS(200));         /* keep watchdog happy */
...
static void mqtt_app_start(void)
{
    ...
    static const char client_id[] = DEVICE_ID;
    ESP_LOGI(TAG, "Using fixed CLIENT_ID: %s", client_id);
    ...
    esp_mqtt_client_register_event(client, ESP_EVENT_ANY_ID, mqtt_event_handler, (void *)client_id);
